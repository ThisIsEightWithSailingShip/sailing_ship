...
public class OrderController {
    ...
    public String getOrderCheckList(Model model,@AuthenticationPrincipal UserDetailsImpl userDetails) { // 인증 객체 메소드 파라미터로 추가 필요
        List<Order> orderCheckList = orderService.getOrderCheckList(userDetails);
    ...
    public ResponseEntity<String> updateOrderStatus(@RequestBody OrderCheckRequestDto orderCheckRequestDto,
                                                    @AuthenticationPrincipal UserDetailsImpl userDetails) {
        System.out.println("주문번호 : "+orderCheckRequestDto.getOrderId());
        orderService.completeOrderCheck(orderCheckRequestDto, userDetails);
